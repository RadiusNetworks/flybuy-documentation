...
The FlyBuy SDK uses push messages to update order state, ETA, and stop location services for a given order. The FlyBuy backend can use Firebase, APNS, OneSignal, Urban Airship to send these push messages to the app, or partners can instead use their own push message service to implement this functionality. If using your own push message service, subscribe to FlyBuy's outgoing webhooks in the merchant portal. These will notify your backend to send the push message (see [Webhooks](webhooks.md)) to your app. In either case, the FlyBuy SDK relies on the app to notify it that a new push message has been received.
